...
(function($){
	...
	$.fn.contactable = function(options) {
		...
		return this.each(function(options) {
			...
			//show / hide function
                        $('div#contactable').attr('showorhide', "false");
			$('div#contactable').toggle(function() {
				$('#overlay').css({display: 'block'});
				$(this).animate({"marginRight": "-=25px"}, "fast"); 
				$('#contactForm').animate({"marginRight": "-=0px"}, "fast");
				$(this).animate({"marginRight": "+=498px"}, "slow"); 
				$('#contactForm').animate({"marginRight": "+=501px"}, "slow");
                                $(this).attr('showorhide', "true");
			}, 
			function() {
				$('#contactForm').animate({"marginRight": "-=501px"}, "slow");
				$(this).animate({"marginRight": "-=498px"}, "slow").animate({"marginRight": "+=25px"}, "fast"); 
				$('#overlay').css({display: 'none'});
                                $(this).attr('showorhide', "false");
			});
					...
					function(data){
						...
						if( data == 'success') {
							...
							if(defaults.hideOnSubmit == true) {
								...
								$('div#contactable').animate({dummy:1}, 2000).animate({"marginRight": "-=447px"}, "slow").animate({"marginRight": "+=25px"}, "fast"); 
								$('#overlay').css({display: 'none'});	
							}
						} else {
