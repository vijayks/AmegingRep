<%--
    ...
    Created on : Mar 3, 2010, 10:35:14 AM
    Author     : Madhuri
--%>
...
<div class="verdana_form_post" style="width:730px">
    <br/>
    <br/> <center><span style="color:red"><c:if test="${exist!=null}">
        ${exist}</c:if></span></center>
          <center><span style="color:green"><c:if test="${ok!=null}">
        ${ok}</c:if></span></center>
<c:if test="${fn:length(productsList)=='0'}">
 <center><span style="font-size:14px;color:red"><b> No product yet created .Hence , no list of Top N list products </b></span></center>

</c:if>
    <%--<c:if test="${productsList!=null}">--%>
        <c:if test="${fn:length(productsList)!='0'}">

    <fieldset><legend style="color:#3366cc"><b>Top N List</b></legend>
    ...
    <form name="assignWCT" action="assignWCT.do?m=in" method="POST" onsubmit="return pId(this)">
            ...
            <tr>
                ...
                <td><select id="prod" name="prod" style="margin-left:20px" onchange="return hello(this)">
                        <option value="0">Select Product</option>
                            ...
                            <c:forEach items="${productsList}" var="pd"> 1
        ...
        <span style="margin-left:150px"><input type="submit" value="Assign"/></span><input type="hidden" name="lT" value="tnl"/>
    </form>
    </fieldset>
 <%--</c:if>--%>
    <br/>
    <br/>
    <%-- <c:if test="${fn:length(productsList)=='0'}">
           <center><span style="font-size:14px;color:#aaaaaa"> No product yet created .Hence , no list of Top N list products</span></center>
     </c:if>--%>
    <fieldset>
        <legend style="color:#3366cc"><b>Existing Top N List</b></legend>
            ...
            <c:if test="${assignList!=null}">
                ...
                <c:forEach items="${assignList}" var="pd">
                    <tr>
                        ...
                        <td><a href="delAssign.do?id=${pd.id}&lT=tnl" ><input type="button" value="Remove"/></a></td>
                    </tr>
